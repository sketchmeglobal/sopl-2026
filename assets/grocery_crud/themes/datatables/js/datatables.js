	...
	if(confirm(message_alert_delete))
	{
		$.ajax({
			...
			success: function(data)
			{
				...
				{
					// success_message(data.success_message); MANUAL UPDATE BY SAYAK ON 20-12-2024
					$('#list-report-error').slideUp('fast');               
					$('#list-report-error').html(data.error_message);
					$('#list-report-error').slideDown('normal');
				}
			}
		});
	}
