...
$(document).ready(function() {
    $('#reveive_purchase_order_table').DataTable({
        ...
        "columns": [
            ...
            { "data": "rate_type" },
            { "data": "buyer_name" },  // Index 3 - searchable but not sortable
            { "data": "quantity" },
            { "data": "total" },
            { "data": "gross_weight" },
            { "data": "net_weight" },
        ...
        "columnDefs": [{
            "targets": [2, 3, 4, 5, 6, 7, 8],  // Buyer (index 3) is included here
            "orderable": false,  // This disables sorting for these columns
        }, {
            "targets": -1,  // targets last column
            "className": 'nowrap'
        }]
    });
});
        ...
        $.confirm({
            ...
            buttons: {
                ...
                print_new: {
                    ...
                    keys: ['enter', 'shift'],
                    action: function(){
                        window.open("<?= base_url() ?>admin/office-invoice-print-tcpdf/"+ $poi, "_blank");
                    }
                },
                printWoSeal: {
                    text: 'Print w/o Seal',
                ...
                articleWeightPrint: {
                    ...
                    keys: ['enter', 'shift'],
                    action: function(){
                        window.open("<?= base_url() ?>admin/office-invoice-print-article-weight/"+ $poi, "_blank");
                    }
                },
    ...
    $(document).on('click', '.json_print_btn',function(){

        $cjid = $(this).attr('cj-id');
        $.confirm({
            title: 'Choose!',
            content: 'Choose Json printing methods from the below options',
            buttons: {
                print: {
                    text: 'With Out Tax',
                    btnClass: 'btn-blue',
                    keys: ['enter', 'shift'],
                    action: function(){
                        window.open("<?= base_url() ?>admin/office-invoice-jsonfile/"+ $cjid, "_blank");
                    }
                },
                print_new: {
                    text: 'With Tax',
                    ...
                    keys: ['enter', 'shift'],
                    action: function(){
                        window.open("<?= base_url() ?>admin/office-invoice-jsonfile-with-tax/"+ $cjid,  "_blank");
                    }
                },
