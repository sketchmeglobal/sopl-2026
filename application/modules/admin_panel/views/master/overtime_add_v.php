    ...
    $("#group").change(function(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                $.each(result, function(index, item) { 
                    const otRate = item.ot_rate ?? 0;
                    const otHours = item.ot_hours ?? 0;
                    const otTotal = item.ot_total ?? 0;
                    const otFactory = Math.round(item.ot_rate_factory ?? 0);
                    const esiPerc = item.esi_percentage ?? 0;
                    const pfCalc = item.pf_percentage_calculation ?? 0;
                
                    let row = `
                        <tr>
                            <td>${item.name}</td>
                            <td><input class="class_rate" type="text" name="add_ot_rate[]" required value="${otRate}" /></td>
                            <td><input class="class_hour" required type="text" name="add_ot_hour[]" value="${otHours}" /></td>
                            <td>
                                <input required class="class_total" type="text" name="add_ot_total[]" value="${otTotal}" readonly />
                                <input type="hidden" name="add_id[]" value="${item.e_id}" />
                                <input type="hidden" name="employee_type[]" class="employee_type" value="${pfCalc}" />
                            </td>
                            <td><input class="fact_act_ot_hrs_max" type="text" name="fact_act_ot_hrs_max[]" value="${otFactory}"/></td>
                            <td><input class="balance_hrs_max" type="text" name="balance_hrs_max[]" value="0" readonly/></td>
                            <td><input class="factory_ot" type="text" name="factory_ot[]" value="0" readonly/></td>
                            <td><input class="bonus_total" type="text" name="add_ot_bonus[]" value="0" readonly/></td>
                            <td><input class="main_total" type="text" name="add_ot_main_total[]" value="0" readonly/></td>
                            <td>
                                <input class="esi_percentage" type="hidden" name="esi_percentage[]" value="${esiPerc}"/>
                                <input class="deduct_esi" type="text" name="add_ot_deduct_esi[]" value="0" readonly/>
                            </td>
                            <td><input class="advnc_oths" type="text" name="add_ot_advnc_oths[]" value="0"/></td>
                            <td><input class="deduct_total" type="text" name="add_ot_deduct_total[]" value="0" readonly/></td>
                            <td><input class="net_pay" type="text" name="add_ot_net_pay[]" value="0" readonly/></td>
                        </tr>`;
                    $("#cutting_bill_details_table_insert tbody").append(row);
                });


            } else {

               $("#cutting_bill_details_table_insert tbody").append('<tr><td colspan="13" align="center">Overtime details already added</td></tr>');
