                    ...
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="tab-content">
                                        ...
                                        <form id="form_add_item_color_rate" method="post" action="<?=base_url('admin/form_add_item_color_rate')?>" class="cmxform form-horizontal tasi-form">
                                            ...
                                            <div class="form-group">
                                                <div class="text-center" style="margin:auto;display:block">
                                                    ...
                                                    <button class="btn btn-success btn-sm" type="submit"><i class="fa fa-plus"></i> Add Item Rate</button>
                                                    <button type="button" id="update_article_sub_btn" class="btn btn-warning btn-sm"><i class="fa fa-recycle"></i>Add Rate & Update Costing</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                ...
                                <!--ARTICLE RATE UPDATE MODAL AREA-->
                                
                                <!-- The modal -->
                                <div class="modal fade" id="update_article_modal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-md" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <h4 class="modal-title" id="modalLabel">Select from the article list</h4>
                                            </div>
                                            <div class="modal-body">
                                                
                                                <form target="_blank" id="form_multiple_costing_print" method="post" action="<?=base_url('admin/print_multiple_article_costing')?>">
                                                    <input type="hidden" id="modal_ac_ids" value="" name="modal_ac_ids[]" />
                                                    <input type="submit" value="Print Cost Sheet" name="costing_on_ac_id" class="btn btn-sm btn-warning pull-right" />
                                                </form>
                                                
                                                <form id="form_add_item_color_rate_on_article" method="post" action="<?=base_url('admin/form-add-item-color-rate-on-article')?>" class="cmxform form-horizontal tasi-form">
                                                    <table id="article_table_on_item_dtl" class="table table-bordered table-striped">
                                                        <thead>
                                                            <tr>
                                                                <th>#</th>
                                                                <th>Article Name</th>
                                                                <th>Cost Sheet</th>
                                                                <th><input type="checkbox" id="trigger_checkbox" /> <label for="trigger_checkbox">Select All</label></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody id="articles_on_item_dtl"></tbody>
                                                    </table>
                                                    
                                                    <!--all values of the insert form-->
                                                    <input type="hidden" name="item_dtl_id_on_article" id="item_dtl_id_on_article" value="">
                                                    <input type="hidden" name="supplier_on_article" id="supplier_on_article" value="">
                                                    <input type="hidden" name="gst_on_article" id="gst_on_article" value="">
                                                    <input type="hidden" name="eff_date_on_article" id="eff_date_on_article" value="">
                                                    <input type="hidden" name="pur_rate_on_article" id="pur_rate_on_article" value="">
                                                    <input type="hidden" name="cost_rate_on_article" id="cost_rate_on_article" value="">
                                                    <input type="hidden" name="plating_rate_add1_on_article" id="plating_rate_add1_on_article" value="">
                                                    <input type="hidden" name="action_type" id="action_type" value="">
                                                    <!--all values of the insert form-->
                                                    
                                                    <button style="display: block;margin: auto;" type="submit" class="btn btn-info">Submit</button>
                                                </form>
                                        ...
                                        <form id="form_edit_item_color_rate" method="post" action="<?=base_url('admin/form_edit_item_color_rate')?>" class="cmxform form-horizontal tasi-form">
                                            ...
                                            <div class="form-group">
                                                <div class="text-center" style="margin:auto;display:block">
                                                    ...
                                                    <input type="hidden" id="item_dtl_id2" name="item_dtl_id2" value="">
                                                    <button class="btn btn-success" type="submit"><i class="fa fa-refresh"></i> Update Item Color Rate</button>
                                                    <button type="button" id="update_article_sub_btn2" class="btn btn-warning btn-sm"><i class="fa fa-recycle"></i>Update Rate & Costing</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
...
<script>
    ...
    $("#form_add_item_color_rate_on_article").validate({
        rules: {
            am_id: {
                required: true,
            },
    ...
    $('#form_add_item_color_rate_on_article').ajaxForm({
        beforeSubmit: function () {
            return $("#form_add_item_color_rate_on_article").valid(); // TRUE when form is valid, FALSE will cancel submit
        },
        success: function (returnData) {
            obj = JSON.parse(returnData);
            notification(obj);
    ...
    // COSTING WHILE ADDING ITEM
    $(document).on('click', '#update_article_sub_btn', function(){
        
        // place it in the modal
        if($(".item_dtl_id").val() == '' || $("#supplier").val() == '' || $("#gst").val() == '' || $("#pur_rate").val() == '' || $("#cost_rate").val() == '' || $("#eff_date").val() == ''){
            alert("Mendatory fields can't be left blank")
            return false
        } else{
        
            item_dtl_id = $(".item_dtl_id").val()
            supplier_on_article = $("#supplier").val()
            gst_on_article = $("#gst").val()
            pur_rate_on_article = $("#pur_rate").val()
            cost_rate_on_article = $("#cost_rate").val()
            eff_date_on_article = $("#eff_date").val()
            plating_rate_add1_on_article = $("#plating_rate_add1").val()
            
            $("#item_dtl_id_on_article").val(item_dtl_id)
            $("#supplier_on_article").val(supplier_on_article)
            $("#gst_on_article").val(gst_on_article)
            $("#pur_rate_on_article").val(pur_rate_on_article)
            $("#cost_rate_on_article").val(cost_rate_on_article)
            $("#eff_date_on_article").val(eff_date_on_article)
            $("#plating_rate_add1_on_article").val(plating_rate_add1_on_article)
            $("#action_type").val('add')
            
            $.ajax({
                url: "<?= base_url('ajax-articles-on-item_dtl') ?>",
                type: 'POST',
                dataType: 'json',
                data:{item_dtl_id: item_dtl_id,supplier_on_article: supplier_on_article, gst_on_article:gst_on_article, pur_rate_on_article:pur_rate_on_article, cost_rate_on_article:cost_rate_on_article, eff_date_on_article:eff_date_on_article, plating_rate_add1_on_article:plating_rate_add1_on_article},
                success: function(returnData){
                    
                    // console.log('.......... articles-on-item_dtl ..........')
                    ...
                    $.each(returnData, function(index, value) {
                        ...
                        link = "<?=base_url('admin/print_article_costing')?>/"+value.ac_id;
                        appnd += "<tr>"
                        appnd += "<td>"+ iter++ +"</td>"
                        appnd += "<td>"+value.art_no+"</td>"
                        appnd += "<td><a target='new' href='"+link+"'>View Cost Sheet</a></td>"
                        appnd += "<td><input class='generated_checkbox' type='checkbox' name='am_id[]' data-ac_id='"+value.ac_id+"' value='"+value.am_id+"'></td>"
                        appnd += "<tr>"
                        
                    });    
                    $('#articles_on_item_dtl').html(appnd)
                ...
                // $("#article_table_on_item_dtl").DataTable();
    ...
    // COSTING WHILE EDITING ITEM
    $(document).on('click', '#update_article_sub_btn2', function(){
        
        // place it in the modal
        if($("#item_dtl_id2").val() == '' || $("#supplier2").val() == '' || $("#gst2").val() == '' || $("#pur_rate2").val() == '' || $("#cost_rate2").val() == '' || $("#eff_date2").val() == ''){
            alert("Mendatory fields can't be left blank")
            return false
        } else{
        
            item_dtl_id = $("#item_dtl_id2").val()
            supplier_on_article = $("#supplier2").val()
            gst_on_article = $("#gst2").val()
            pur_rate_on_article = $("#pur_rate2").val()
            cost_rate_on_article = $("#cost_rate2").val()
            eff_date_on_article = $("#eff_date2").val()
            plating_rate_add1_on_article = $("#plating_rate_add12").val()
    ...
    $(document).on('click', '#trigger_checkbox', function(){
        if ($(this).is(':checked')) {
            $(".generated_checkbox").prop('checked', true)
            // For printing
            $("#modal_ac_ids").val("")
            $(".generated_checkbox").each(function() {
                    
                if ($(this).is(':checked')) {
                
                    ac_id = $(this).data('ac_id')
                    if($("#modal_ac_ids").val() == ''){
                        $("#modal_ac_ids").val(ac_id)
                    } else{
                        cur_val = $("#modal_ac_ids").val()
                        new_val = cur_val + ',' + ac_id
                        $("#modal_ac_ids").val(new_val)
                    }
            ...
            $(".generated_checkbox").prop('checked', false)
            $("#modal_ac_ids").val("")
        }
        
    })
    
    $(document).on('click', '.generated_checkbox', function(){
