<?PHP
    ...
    function check_negative_qnty($co_id, $am_id, $lc_id, $pack_list_quantity){
        
        $CI =& get_instance();
        $query = "SELECT SUM(jobber_receive_quantity) AS jobber_receive_quantity FROM jobber_challan_receipt_details WHERE co_id=$co_id AND am_id=$am_id AND lc_id=$lc_id";
        $jobber_receive_quantity = $CI->db->query($query)->row()->jobber_receive_quantity;
        if($jobber_receive_quantity == '' or $jobber_receive_quantity == NULL){
            $jobber_receive_quantity = 0;
        }
        
        
        $query = "SELECT SUM(article_quantity) AS article_quantity FROM packing_shipment_detail WHERE co_id=$co_id AND am_id=$am_id AND lc_id=$lc_id AND status=1";
        $packing_article_quantity = $CI->db->query($query)->row()->article_quantity;
        if($packing_article_quantity == '' or $packing_article_quantity == NULL){
            $packing_article_quantity = 0;
        }
        
        // echo $jobber_receive_quantity . '...' . $packing_article_quantity; die;
        
        return ($jobber_receive_quantity - $packing_article_quantity);
        
        // return $co_id . ' ... ' . $am_id . ' ... ' . $lc_id . ' ... ';
    }
...
<html lang="en">
    ...
    <head>
        <meta charset="utf-8">
        <title>NEGETIVE SHIPMENT LIST | <?=WEBSITE_NAME;?></title>
            ...
            <div>
                ...
                <div class="container">
                    ...
                    <div class="row border_all text-center text-uppercase mar_bot_3">
                        <h3 class="mar_0 head_font">NEGATIVE SHIPMENT LIST</h3>
                    </div>
                    ...
                    <div class="row table-responsive">
                        ...
                        <table class="table table-bordered table-hover table2excel">
                            ...
                            <tbody class="actual_table">
                                <?php
                                    // echo '<pre>', print_r($print_packing_details), '</pre>';die();
                    ...
                    <div class="row table-responsive">
                        ...
                        <table class="table table-bordered table-hover table2excel">
                            ...
                            <?php 
                                ...
                                if(check_negative_qnty($ppl->co_id, $ppl->am_id, $ppl->lc_id, $ppl->article_quantity) == 0){
                                    continue;
                                }
                                if(check_negative_qnty($ppl->co_id, $ppl->am_id, $ppl->lc_id, $ppl->article_quantity) > 0){
                                    $class = "";
                                } else{
                                    $class = "bg-red text-danger fw-bold";
                                }
                            ?>
                                    <tr>
                                        <td><?= $ppl->co_no ?></td>
                                        ...
                                        <td style="text-align: center!important" class="<?=$class?>"><?= check_negative_qnty($ppl->co_id, $ppl->am_id, $ppl->lc_id, $ppl->article_quantity) ?></td> 
                                    ...
                                    if(end($keys) == $curr_key) {
                                        ...
                                        <!--    <td style="font-weight: bold; font-size: 14px; text-align: center!important" class="text-right"><?= $groups[$ppl->co_no]['gr_sum'] ?></td>-->
                                        <!--</tr>-->
                                        <?php
                                    }
                                ?>
