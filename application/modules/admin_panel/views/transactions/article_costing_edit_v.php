...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <div class="wrapper">
                    ...
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <form id="form_edit_article_costing" method="post" action="<?=base_url('admin/form_edit_article_costing')?>" class="cmxform form-horizontal tasi-form">
                                ...
                                <div class="form-group ">
                                    <label class="control-label col-lg-2">Cost Increment</label>
                                    <div class="col-lg-4 text-center">
                                        <input id="cost_increment" name="cost_increment" type="number" placeholder="Cost Increment" class="form-control round-input" value="<?= $article_costing->cost_increment ?>" />
                                    </div>
            ...
            <?php #echo '<pre>',print_r($measurement_wastage),'<pre>';  ?>
            
            <div class="row">
                <div class="col-md-12">
                    <section class="panel">
                        <header class="panel-heading">
                            Exchange Wastage from Measurement / Costing:
                            <span class="tools pull-right">
                                <a class="t-collapse fa fa-chevron-down" href="javascript:;"></a>
                            </span>
                        </header>
                        <div class="panel-body">
                            <div class= "row form-group">
                                <label for="existing_wastage_percentage" class="control-label col-lg-2 text-danger"> Existing Wastage*</label>
                                <div class="col-lg-3">
                                    <select id="existing_wastage_percentage" name="existing_wastage_percentage" required class="select2 form-control round-input">
                                        <option value="" item_group_val="" unit="">Select From The List</option>
                                        <?php
                                        foreach($measurement_wastage as $mi) {
                                            ?>
                                            <option wastage_percentage_str="<?= $mi['wastage_percentage'] ?>" value="<?=$mi['ac_id']?>"><?= $mi['wastage_percentage'] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                
                                <label for="proposed_wastage" class="control-label col-lg-2 text-danger text-right"> Proposed Wastage</label>
                                <div class="col-lg-3">
                                    <input id="proposed_wastage" type="number" value="" placeholder="Proposed Wastage" required style="text-align:left" class="proposed_wastage form-control round-input" />
                                </div>
                                <div class="col-lg-2 text-right">
                                    <button class="btn btn-success" type="button" id="swap_wastage"><i class="fa fa-exchange"></i> Group Alter</button>
                                </div>
...
return false;
    ...
    //
    //swap on wastage change
    $("#swap_wastage").on('click', function(){
        wastage_percentage = $('#existing_wastage_percentage').select2('data').text 
        costing_id = $('#existing_wastage_percentage option:selected').val()
        proposed_wastage = $('#proposed_wastage').val();
        // alert(wastage_percentage + ' ... ' + costing_id + ' ... ' + proposed_wastage)
        
        $.ajax({
            url: "<?= base_url('admin/alter-costing-on-wastage') ?>",
            dataType: 'json',
            method: 'POST',
            data: {wastage_percentage : wastage_percentage, costing_id: costing_id, proposed_wastage: proposed_wastage},
            success: function(obj){
                ...
                $('#costing_details_table').DataTable().ajax.reload();
                // open the item tray 
                $('#existing_wastage_percentage').select2('open');
            },
            error: function(e){
                console.log(e);
            }
