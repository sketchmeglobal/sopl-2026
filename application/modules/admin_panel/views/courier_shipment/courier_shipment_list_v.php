...
<html lang="en">
<head>
    <title>Courier Shipment List | <?=WEBSITE_NAME;?></title>
...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <!-- page head start-->
        <div class="page-head">
            <h3 class="m-b-less">Courier Shipment</h3>
            <div class="state-information">
                <ol class="breadcrumb m-b-less bg-less">
                    <li><a href="<?=base_url('admin/dashboard');?>">Home</a></li>
                    <li class="active"> Courier Shipment List </li>
                </ol>
            </div>
        </div>
        <!-- page head end-->
                        ...
                        <div class="panel-body">
                            <?php 
                                if($view_permission != 'block'){
                                    ?>
                            <a href="<?= base_url('admin/add-courier-shipment') ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add courier Shipment List</a>
                                <?php
                            } 
                            ?>
...
<script>
    $(document).ready(function() {
        $('#cutting_issue_challan_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('admin/ajax-courier-shipment-list-table-data')?>",
                "type": "POST",
                "dataType": "json",
            },
            ...
            "columns": [
                ...
                { "data": "total_quantity" },
                { "data": "total_value" },
                { "data": "total_foreign_value" },
                { "data": "action" },
            ],
            //column initialisation properties
            "columnDefs": [{
                "targets": [7], //disable 'Image','Actions' column sorting
                "orderable": false,
            }]
        } );
    } );
    ...
    $(document).on('click', '.delete', function(){
        if(confirm('Are you sure?')){
            ...
            $.ajax({
                url: "<?= base_url('admin/delete-courier-shipment-header-list') ?>",
                type: 'POST',
                dataType: 'json',
