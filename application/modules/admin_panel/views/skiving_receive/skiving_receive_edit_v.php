 ...
 * Date: 11-03-2020
 * Time: 09:55
 * Last updated on 21-01-2021 at 12:30pm
 */
 ?>
...
<head>
    ...
    <meta name="description" content="Skiving Receive Edit ">
                    ...
                    <section class="panel">
                        ...
                        <?php #print_r($purchase_order_details); die;?>
                            ...
                            <div class="form-group ">
                                ...
                                <!--<input type="hidden" value="" id="cut_recieve_id" />-->
                            ...
                            <div class="tab-content">
                                ...
                                <div id="cut_issue_challan_list" class="tab-pane fade in active">
                                    <table id="cutting_receive_challan_details_table" class="table data-table dataTable">
                                        <thead>
                                            <tr>
                                                <th>Skiving Issue Challan Number</th>
                                        ...
                                        <form id="form_add_skiving_receive_challan_details" method="post" action="<?=base_url('admin/form-add-skiving-receive-challan-details')?>" class="cmxform form-horizontal tasi-form">
                                            <div class="form-group ">
                                                ...
                                                <div class="col-lg-4"><!-- It is showing Article Number[Atricle Details] but calculating with the customer order detail PK -->
                                                    <label for="cut_rcv_detail_id" class="control-label text-danger">Article Number*</label>
                                                    <?php //print_r($articles); ?>
                                                    <select id="cut_rcv_detail_id" name="cut_rcv_detail_id" class="form-control select2">
                                                     ...
                                                     <input type="hidden" id="co_id" name="co_id" required class="form-control" readonly />
                                                </div>
                                                <div class="col-lg-2">
                                                ...
                                                <div class="col-lg-2">
                                                    ...
                                                    <input type="hidden" id="receive_quantity_add_hidden" name="receive_quantity_add_hidden" required class="form-control" />
...
<script>
    ...
    $(document).ready(function() {
        $('#cutting_receive_challan_details_table').DataTable( {
            ...
            "columns": [
				{ "data": "skiving_issue_challan_number" },
				{ "data": "article_number" },
                { "data": "leather_color" },
                { "data": "fitting_color" },
	...
	$(document).on('change', '#cut_rcv_id', function(){
        $cut_rcv_id = $(this).val();
        $.ajax({
            url: "<?= base_url('admin/fetch-all-skiving-issue-article') ?>",
            method: "post",
            dataType: 'json',
            data: {'cut_rcv_id': $cut_rcv_id,},
            success: function(result){
                var article = result.articles;
				
                $("#cut_rcv_detail_id").html("<option value=''> Select Article</option>");
                $.each(article, function(index, item) {
                    $str = '<option value=' + item.cut_rcv_detail_id + ' am_id=' + item.am_id + ' co_id=' + item.co_id + ' cod_id=' + item.cod_id + ' fitting_color=' + item.fitting_color + ' fitting_id=' + item.fitting_id + ' leather_color=' + item.leather_color + ' leather_id=' + item.leather_id + ' cut_rcv_id=' + item.cut_rcv_id + '> '+ item.art_no + '['+ item.leather_color +'] ['+ item.co_no +']</option>';
                    $("#cut_rcv_detail_id").append($str);
                });
                // open the item tray 
                $('#cut_rcv_detail_id').select2('open');
            },
            error: function(e){console.log(e);}
        });
    });
	
	$("#cut_rcv_detail_id").change(function(){
        $cut_rcv_detail_id = $(this).val();
		...
		$cut_recieve_id = $('#cut_recieve_id').val();
        ...
        $.ajax({
            ...
            method: "post",
            dataType: 'json',
            data: {'am_id': $am_id, 'cut_rcv_id': $cut_rcv_id, 'cod_id': $cod_id, 'cut_recieve_id': $cut_recieve_id, 'lc_id': $leather_id},
            success: function(result){
                console.log(JSON.stringify(result));
				$('#receive_cut_quantity_add').val(result.issue_quantity_in_skiving);				
				$('#receive_quantity_add').val(result.remain_receive_quantity_in_skiving);
				$('#receive_quantity_add_hidden').val(result.remain_receive_quantity_in_skiving);
            },
            error: function(e){console.log(e);}
        });
    });
	...
	$("#receive_quantity_add").blur(function(){
		...
		$receive_quantity_add_hidden = $('#receive_quantity_add_hidden').val();
		if(parseInt($receive_quantity_add) > parseInt($receive_quantity_add_hidden)){
			alert('Can not greater than Receive Quantity: '+$receive_quantity_add_hidden);
...
<script>$("#skiving_add").click(function(){	
//$("#cut_rcv_detail_id").select2("destroy").val(1).trigger("change").select2();
})</script>
</body>
</html>
