                    ...
                    <section class="panel">
                        <div class="panel-body">
                            <form class="" method="post" target="_blank">
                                <div class="row">
                                    <div class="col-sm-4">
                                        <!--<label for="supp_second_2" class="control-label text-danger">Select Group*</label>-->
                                        <!--<select id="supp_second_2" name="supp_second_2" required class="form-control select2">-->
                                        <!--    <option value="">Select Group</option>-->
                                        <!--        < ?php-->
                                        <!--        foreach ($group_arrays as $g_a) {-->
                                        <!--        ?>-->
                                        ...
                                        <!--<hr>-->
                                        <label class="control-label text-danger">Select Buyer*</label>
                                        <select id="buyer_wise_order_details_2" name="buyer_wise_order_details_2" class="form-control select2">
                                            <option value="">Select Buyer</option>
                                            <?php foreach ($buyers as $buyer) {
                                                ?>
                                                <option value="<?= $buyer->am_id ?>"><?= $buyer->name . '['. $buyer->short_name .']' ?></option>
                                    ...
                                    <div class="col-sm-5">
                                        <label>Select Article  </label><br />
                                        <a id="select-all_2" href="#">Select All</a>
                                        /
                                        <a id="deselect-all_2" href="#">Deselect All</a>
                                        <select id="costing_gropus_2" class="form-control" name="gropus_2[]" multiple="multiple"></select>
                                    </div>
                                    <div class="col-sm-3">
                                        <label>Action </label><br />
                                        <input type="submit" name="supplier_wise_cost_position" value="Print" class="btn btn-sm btn-success btn-lg" />
                                    </div>
                                </div>
                            </form>
                            <br>
...
<script type="text/javascript" src="<?=base_url()?>assets/admin_panel/js/jquery.quicksearch.js"></script>
    ...
    <script>
        ...
        $('#select-all_2').click(function(){
           $('#costing_gropus_2').multiSelect('select_all');
           return false;
        });
        $('#deselect-all_2').click(function(){
           $('#costing_gropus_2').multiSelect('deselect_all');
           return false;
        });
    </script>
       
    <script>
        $(document).on('change', '#buyer_wise_order_details_2', function(){
              
            $("#costing_gropus_2").html('');
            $am_id1 = $(this).val();
            // $am_id = $("#supp_second_2").val();
            // alert($im_id);

            $.ajax({
                url: "<?= base_url('admin/get-fetch-all-item-for-costings-article-on-buyer') ?>",
                method: "post",
                dataType: 'json',
                data: {'am_id1': $am_id1},
                success: function(all_colors){
                    ...
                    $.each(all_colors, function(index, item) {
                        str = '<option value=' + item.am_id + '> '+ item.art_no + '</option>';
                        $("#costing_gropus_2").append(str);
                    });
                    $('#costing_gropus_2').multiSelect('refresh');
                },
                error: function(e){console.log(e);}
            });
