                ...
                <div class="col-lg-12">
                    <section class="panel">
                        <div class="panel-heading">
                            <p style="margin:0" class="">Customer order wise leather status</p>
                        </div>
                        <div class="panel-body">
                            <form class="row" method="post" action="" target="_blank">
                                <div class="col-lg-3">
                                    <label>Select Customer Order </label><br />
                                    <select multiple id="co" name="co" class="select2 form-control" required >
                                        <?php foreach($fetch_all_co as $fac){ ?> 
                                            <option value="<?=$fac->co_id?>"><?=$fac->co_no?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-lg-6">
                                    <label>Select Item From The List</label><br />
                                    <a id="select-all-co" href="#">Select All</a>
                                    /
                                    <a id="deselect-all-co" href="#">Deselect All</a>
                                    <select id="leather_status_co" name="leather[]" multiple="multiple" style="width: 100%">
                                    </select>
                                </div>
                                <div class="col-lg-3">
                                    <label for="">Action</label><br>
...
<script>
    ...
    //   co area
        $("#co").change(function(){
            ...
            $.ajax({
               method: 'post',
               dataType: 'json',
               url: "<?= base_url('ajax-items-on-customer-order') ?>",
               data: {co: co},
               success: function(items){
                    console.log('----ORIGINAL CO.----');
                    console.log(items);
                    $("#leather_status_co").html("")
                    $.each(items, function(index, itemData){
                        $.each(itemData, function(index, value){
                            $apnd_val = '<option value="'+ value.id_id +'">'+ value.item + ' [' + value.color + ']' +'</option>';
                            $("#leather_status_co").append($apnd_val);
                        })
                       
                    //   return false;
                    });
                    $('#leather_status_co').multiSelect('refresh');
               },
               error: function(e){
                   console.log(e);
               }
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#select-all-co').click(function(){
            $('#leather_status_co').multiSelect('select_all');
               return false;
            });
            $('#deselect-all-co').click(function(){
               $('#leather_status_co').multiSelect('deselect_all');
               return false;
            });
    </script>
