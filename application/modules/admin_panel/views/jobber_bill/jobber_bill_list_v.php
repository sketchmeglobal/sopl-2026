...
<html lang="en">
<head>
    <title>Jobber Bill List | <?=WEBSITE_NAME;?></title>
...
<section>
    ...
    <div class="body-content" style="min-height: 1500px;">
        ...
        <!-- page head start-->
        <div class="page-head">
            <h3 class="m-b-less">Jobber Bill</h3>
            <div class="state-information">
                <ol class="breadcrumb m-b-less bg-less">
                    <li><a href="<?=base_url('admin/dashboard');?>">Home</a></li>
                    <li class="active"> Office Proforma </li>
                </ol>
            </div>
        </div>
        <!-- page head end-->
                        ...
                        <div class="panel-body">
                            <?php 
                                if($view_permission != 'block'){
                                    ?>
                            <a href="<?= base_url('admin/add-jobber-bill') ?>" class="btn btn-success"><i class="fa fa-plus"></i> Add Jobber Bill</a>
                                <?php
                            } 
                            ?>
...
<script>
    $(document).ready(function() {
        $('#cutting_issue_challan_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('admin/ajax-jobber-bill-table-data')?>",
                "type": "POST",
                "dataType": "json",
            },
    ...
    $(document).on('click', '.update_art_master_from_jobber_bill', function(){
        if(confirm('Are you sure?')){
            $pk_value = $(this).attr('pk-value');
            
            
            $.ajax({
                url: "<?= base_url('admin/jobber-bill-update-article-master') ?>",
                type: 'POST',
                dataType: 'json',
    ...
    $(document).on('click', '.delete', function(){
        if(confirm('Are you sure?')){
            ...
            $.ajax({
                url: "<?= base_url('admin/delete-jobber-bill-header-list1') ?>",
                type: 'POST',
                dataType: 'json',
