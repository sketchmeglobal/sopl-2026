                                ...
                                <div id="po_add" class="tab-pane fade">
                                    ...
                                    <div class="form">
                                        <form id="form_add_purchase_order_details" method="post" action="<?=base_url('admin/form_add_purchase_order_details')?>" class="cmxform form-horizontal tasi-form">
                                            ...
                                            <div class="form-group ">
                                                <div class="col-lg-12">
                                                    <label for="co_ids" class="control-label">Customer Order(s)</label>
                                                    <select id="co_ids" name="co_ids[]" class="select2 form-control round-input" multiple>
                                                        <option value="">Select Customer Order</option>
                                                        <?php foreach($co_ids as $co) { ?>
                                                            <option value="<?=$co->co_id?>"><?=$co->co_no . (!empty($co->buyer_reference_no) ? ' [' . $co->buyer_reference_no . ']' : '')?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            </div>
                                ...
                                <div id="po_details_edit" class="tab-pane">
                                    ...
                                    <div class="form">
                                        <form id="form_edit_purchase_order_details" method="post" action="<?=base_url('admin/form_edit_purchase_order_details')?>" class="cmxform form-horizontal tasi-form">
                                            ...
                                            <div class="form-group ">
                                                <div class="col-lg-12">
                                                    <label for="co_ids_edit" class="control-label">Customer Order(s)</label>
                                                    <select id="co_ids_edit" name="co_ids_edit[]" class="select2 form-control round-input" multiple>
                                                        <option value="">Select Customer Order</option>
                                                        <?php foreach($co_ids as $co) { ?>
                                                            <option value="<?=$co->co_id?>"><?=$co->co_no . (!empty($co->buyer_reference_no) ? ' [' . $co->buyer_reference_no . ']' : '')?></option>
                                                        <?php } ?>
    ...
    $("#po_details_table").on('click', '.purchase_details_edit_btn', function() {
        ...
        $.ajax({
            ...
            success: function(pod_data){
                ...
                // Set customer orders
                if(data.co_ids && data.co_ids !== ''){
                    var coIdArray = data.co_ids.split(',');
                    $("#co_ids_edit").val(coIdArray).trigger('change');
                } else {
                    $("#co_ids_edit").val(null).trigger('change');
                }
