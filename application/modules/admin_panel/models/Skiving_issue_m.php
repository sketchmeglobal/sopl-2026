	...
	**/
    ...
    public function ajax_skiving_bill_table_data() {
        ...
        foreach ($rs as $val) {
            ...
            $nestedData['total_amount'] = $this->db->select('SUM(skiving_amount) as total')->get_where('skiving_bill_detail', array('sb_id' => $val->sb_id))->row()->total;
    ...
    public function form_add_skiving_bill(){
        ...
        $sbn = $this->input->post('skiving_bill_number');
        $rs = $this->db->get_where('skiving_bill', array('bill_number' => $sbn))->num_rows();
        if($rs > 0){
            $data['type'] = 'error';
