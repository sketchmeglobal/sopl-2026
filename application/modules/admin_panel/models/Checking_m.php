...
//         return $data;
//     }
        //22-02-25 update form_checking_list_add() this function from tanay for same date validation add
        public function form_checking_list_add(){
            $checking_entry_date = $this->input->post('checking_start_date_time');
            $e_id = $this->input->post('e_id');
            $this->db->where('e_id', $e_id);
            $this->db->where('checking_entry_date', $checking_entry_date);
            $query = $this->db->get('checking');
        
            if ($query->num_rows() > 0) {
                $data['type'] = 'error';
                $data['msg'] = 'This employee already has an entry for the selected date.';
            } else {
                $insertArray = array(
                    'extra_time' => $this->input->post('extra_time'),
                    'e_id' => $e_id,
                    'checking_entry_date' => $checking_entry_date,
                    'user_id' => $this->session->user_id
                );
                ...
                if($this->db->insert_id() > 0){
                    ...
                    $data['type'] = 'error';
                    $data['msg'] = 'Failed to insert record.';
                }
            }
            return $data;
        }
