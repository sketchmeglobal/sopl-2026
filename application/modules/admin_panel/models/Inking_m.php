...
//          print_r($rs); die();
    ...
    public function get_customer_order_dtl_for_inking_am_id(){
        ...
        $org_category_qnty = $this->input->post('org_category_qnty');
        ...
        if($remarks_for_other_quantity == 'OTHERS') { 
            ...
            if($checked_quantity == '' or $checked_quantity == 0 or $checked_quantity == null){
                $remaining_inking_quantity = ($co_quantity*$org_category_qnty);
            }else{
                $category_quantity = $this->db
                    ->select_sum('category_quantity')
                    ->get_where('inking_details', array('cod_id' => $cod_id, 'am_id' => $am_id, 'remarks_for_other_quantity' => $remarks_for_other_quantity, 'inking_category' => $inking_category))
                    ->result()[0]->category_quantity;
                $remaining_inking_quantity = (($co_quantity*$category_quantity) - $checked_quantity);                    
            }
