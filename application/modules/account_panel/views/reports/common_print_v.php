		...
		<div>
			...
			<div class="container">
                ...
                <!-- <tr>
                	...
                	$total_el = 0;
                	$total_esil = 0;
      $current = strtotime('2024-04-1');
      $date2 = strtotime('2025-03-1');
      $stepVal = '+1 month';
      while( $current <= $date2 ) {
...
<?php if($segment == 'payroll_register' ) {
        ...
        <div id="page-content">
            ...
            <div style="text-align: center; margin: 20px 0;">
                <button onclick="exportToExcel()" style="padding: 10px 20px; background-color: #4CAF50; color: white; border: none; cursor: pointer; font-size: 16px; border-radius: 4px;">
        ...
        <body class="A3 landscape" style="overflow-x: auto; padding-top: 20px">
            ...
            function exportToExcel() {
                try {
                    // Get the company name and month for filename
                    var month = '<?= isset($mont) ? $mont : date("Y-m") ?>';
                    var filename = 'Payroll_Register_' + month.replace(/\s+/g, '_') + '.xlsx';
                    
                    // Create a new workbook
                    var wb = XLSX.utils.book_new();
                    
                    // Get all tables with id 'all_det'
                    var tables = document.querySelectorAll('#all_det');
                    ...
                    tables.forEach(function(table, index) {
                        ...
                        var tableData = XLSX.utils.sheet_to_json(
                            XLSX.utils.table_to_sheet(table), 
                            {header: 1, raw: false}
                        );
                        
                        if (index === 0) {
                            // Add all rows from first table including header
                            allData = allData.concat(tableData);
                            headerAdded = true;
                        } else {
                            // For subsequent tables, skip the header rows (first 3 rows typically)
                            // and only add data rows
                            var dataRows = tableData.slice(3); // Adjust this number based on your header rows
                    ...
                    var colWidths = [
                        ...
                        {wch: 12}, // D.A. (Scale)
                        {wch: 12}, // HRA (Scale)
                        {wch: 12}, // Conv. Alwnc (Scale)
                        {wch: 12}, // Med. Alwnc (Scale)
                        {wch: 12}, // Total (Scale)
                        {wch: 12}, // Basic (Paid)
                        {wch: 12}, // D.A. (Paid)
                        {wch: 12}, // HRA (Paid)
                        {wch: 12}, // Conv. Alwnc (Paid)
                        {wch: 12}, // Med. Alwnc (Paid)
                        {wch: 12}, // Total (Paid)
                        {wch: 12}, // P.F.
                        {wch: 12}, // E.S.I.
                        {wch: 12}, // WB. PR TAX
                        {wch: 12}, // L.I.C./T.D.S.
                        {wch: 12}, // Loan/Advance
                        {wch: 12}, // Total Deductions
                        {wch: 15}, // Salary/Wages Paid
                        {wch: 20}  // Sign./Thumb Impression
                    ];
                    ws['!cols'] = colWidths;
...
body.A3.landscape .sheet {
    width: 430mm;
		...
		<div>
			...
			<?php //echo"<pre>"; print_r($resultss); echo"</pre>"; ?>
...
?></b></td>
                                    ...
                                    <td style="text-align: center;">EDU ALLOW / MED. ALLOW  <br/> শিক্ষা ভাতা / চিকিৎসা ভাতা
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center; font-size: 15px;"><?=number_format($result->OA + $result->MED, 2) ?></td>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center;">LOANS / ADVANCE <br/> অ্যাডভানস </td>
                ...
                $current = strtotime('2025-04-01');
                //   $date2 = strtotime('2023-03-01');
                if($wordArray[2] < 4 || $wordArray[2] == 12) {
                   $date2 = strtotime(date('Y').'-' . $mn . '-01'); 
                } else {
                  $date2 = strtotime('2025-' . $mn . '-01');  
                }
                if($wordArray[2] < 4 || $wordArray[2] == 12) {
                $date_prev = strtotime('2025-' . '01' . '-01');     
                } else if($wordArray[2] == 1) {
                $date_prev = strtotime('2025-' . $mn_prev . '-01');
                }else {
                $date_prev = strtotime('2025-' . $mn_prev . '-01');    
                }
                $stepVal = '+1 month';
...
?>
                ...
                $current_sal = strtotime('2025-04-01');
                $dates_sal = array();
                $dates1_sal = array();
                //$date2 = strtotime('2023-03-01');
                ...
                if($array_sal[2] < 4 || $array_sal[2] == 12) {
                  ...
                  $date2_sal = strtotime('2025-' . $mn . '-01');  
                }
...
?>
                                    ...
                                    <?php //echo $salary_details_advn_advc_value; echo"<br>"; 
                                    if ($salary_details_advn_advc_value > 0)  // $result->LOAN > 0 or $salary_details_advn_advc_value > 0
                { ?>
                ...
                { ?>
                                    ...
                                    <?php 
                                    //echo $total_loan; echo"<br>"; 
                                    if ($salary_details_advn_advc_value > 0)  //or $salary_details_advn_advc_value > 0 
                                    
                { ?>
                                    <?php //print_r($total_loan); echo"<br>";
                                    //print_r($salary_details_advn_advc_value);
                                    $salary_advance_total_loan = $total_loan - $salary_details_advn_advc_value; ?>
                                    <td style="text-align: center; font-size: 16px;"><b><?=number_format(abs($salary_advance_total_loan) , 2)?></b></td> 
                                    <?php
                }
                else
                { ?>
